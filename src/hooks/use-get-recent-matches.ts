...
export const useGetRecentMatches = (type: MatchType = "", limit: number = 5) => {
    ...
    const getMatches = useCallback(async () => {
        try {
            const { data } = await getRecentMatches(type, 1, limit);
    ...
    const fetchMore = useCallback(async () => {
        ...
        try {
            ...
            const { data } = await getRecentMatches(type, page + 1, limit);
            if (data?.data?.result?.length > 0) {
